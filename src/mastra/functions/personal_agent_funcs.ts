  ...
  try {
    ...
    for (const draft of messages) {
      try {
        ...
        draftemails.push({ id: draft.id!, subject, from, snippet });
      } catch (err) {
 ...
 */
export async function sendMessage(options: SendMessageInput): Promise<SendMessageOutput> {
  try {
    // Validate input
    const validatedOptions = SendMessageInputSchema.parse(options);
    const { userId = "me", raw, to, subject, body } = validatedOptions;
